    ...
    public function bookRide(Request $request)
    {
        ...
        if ($activeBookingsCount >= 3) {
            return redirect()->back()->with('error',
                'You cannot book more than 3 booking at a time. Please wait for your current booking to be completed or cancelled.');
        }
        ...
        $message = 'Ride booked successfully! Fare: ₱' . number_format($fare, 2);
        if ($request->filled('scheduleTime')) {
            $message .= ' (Scheduled for ' . \Carbon\Carbon::parse($request->scheduleTime)->format('M d, Y h:i A') . ')';
        } else {
...
public function getPendingBookings()
{
    ...
    $bookings = Booking::where('passengerID', $passenger->id)
        ...
        ->map(function($booking) {
            // Get driver ID from the relationship if it exists
            ...
            return [
                ...
                'can_cancel' => in_array($booking->status, ['pending', 'accepted']),
                'can_edit' => $booking->status === 'pending',
                'can_track' => $booking->status === 'in_progress'
            ];
        });
