...
// Store accepted bookings in localStorage to persist across page refreshes
...
function loadAvailableBookings() {
    ...
    // Only load if driver is available
...
// Load accepted bookings with persistence
function loadAcceptedBookings() {
    fetch("{{ route('driver.getAcceptedBookings') }}")
        ...
        .then(data => {
            ...
            // Update localStorage with current accepted bookings
...
// Fallback function to load accepted bookings from localStorage
...
// Check if job can be started (for scheduled bookings)
...
function startJob(bookingId) {
    ...
    .then(data => {
        if (data.success) {
            // Remove from accepted bookings in localStorage
...
// Cancel accepted booking function
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load accepted bookings (will use localStorage as fallback)
    loadAcceptedBookings();
    
    // Load bookings if driver is available
    if (document.getElementById('availabilityToggle').checked) {
